...
int main()
{
	...
	for ( double TEMP = LTEMP; TEMP <= HTEMP; TEMP += STEP )
	{
		...
		double eqconst = AVOGADRO / ( UGASCONST * TEMP ) * Qtr_complex / pow(Q_N2, 2) * result.value() * PATOATM / (16 * pow(M_PI, 5)) / 2; // 2 comes from degeneracy of Psi (Euler angle); \psi integration goes from 0 to \pi instead of 0 to 2 \pi 
